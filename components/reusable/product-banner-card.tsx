...
function ProductBanner({ url, imageurl, title }: Ibanner) {
  ...
  return (
    <div
      className={`banner-wrappper relative w-full min-w-[190px] sm:w-[230px] h-[82px] sm:h-[256px] bg-black `}
    >
      <Image
        src={imageurl}
        fill
      ...
      <div className="absolute  z-1 w-full flex flex-col py-2 gap-2 justify-center items-center text-white top-[50%] transform -translate-y-[50%]">
        ...
        <Link href={url} className="banner-link text-xs font-normal text-center underline hover:text-green-500">
          See All Products
        </Link>
      </div>
    </div>
